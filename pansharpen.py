...
class pansharpen(object): 
  ...
  def __init__(self,outputDirectory,red,green,blue,pan,NIR=None):
    ...
    self.blue = blue
    self.pan = pan
    if not NIR is None: self.NIR = NIR
  ...
  def pansharpenPCA(self): 
    ...
    if self.NIR is not None:
      ...
      NIR = self.NIR 
      image = np.zeros( (nrows,ncols,4),dtype=np.float16)
      image[:,:,0] = self.red
      image[:,:,1] = self.green
      image[:,:,2] = self.blue 
      image[:,:,3] = self.NIR
      ...
      image = np.zeros( (nrows,ncols,3) , dtype=np.float16)
      image[:,:,0] = self.red
      image[:,:,1] = self.green 
      image[:,:,2] = self.blue
    ...
    [ PCAData , PCAMap ] = pansharpen.multiPCA( M, self.outDir, (m*n,d), d )
  ...
  @staticmethod
  def multiPCA( X , outDir, shape, no_dims=None ):
    '''
  ...
  def pansharpenWavelet(self):
    ...
    filename_fp_reconstvec = os.path.join(self.outDir,'reconstvec.dat')
    fp_reconstvec = np.memmap(filename_fp_reconstvec,dtype='float16',mode='w+',shape=(panvec.shape[0],d))
    fp_reconstvec[:] = np.tile( panvec.T, (d,1)).T
    ...
    for j in range( 0 , coeff_shapes[0][0] * coeff_shapes[0][1] ):
      fp_reconstvec[ j,: ] = lowresvec[j,:]

    filename_fp_sharpened = os.path.join(self.outDir,'sharpened_wavelet.dat')
    fp_sharpened = np.memmap( filename_fp_sharpened,dtype='float16',mode='w+',shape=(nrows,ncols,d))

    for band in range(d):
      p = np.reshape( fp_reconstvec[:,band], (nrows,ncols))
      fcoeffs = pywt.wavedec2(p,wavelet_type,level=level)
      out=pywt.waverec2(fcoeffs,wavelet_type)
      fp_sharpened[:,:,band] = out

    redsharp   = fp_sharpened[:,:,0]
    greensharp = fp_sharpened[:,:,1]
    bluesharp  = fp_sharpened[:,:,2]
    
    if d == 4:
      NIRsharp = fp_sharpened[:,:,3]
      del fp_sharpened,fp_reconstvec
      return [redsharp,greensharp,bluesharp,NIRsharp]
    elif d == 3:
      del fp_sharpened,fp_reconstvec
      return [redsharp,greensharp,bluesharp]
...
def main():
  ...
  try: 
    options,arguments = getopt.getopt(sys.argv[1:],'h:vpm:',['help','version',\
  ...
  fpRed_filename   = os.path.join( outputDir , 'red.dat'   )
  fpGreen_filename = os.path.join( outputDir , 'green.dat' )
  fpBlue_filename  = os.path.join( outputDir , 'blue.dat'  )
  fpPan_filename   = os.path.join( outputDir , 'pan.dat'   )

  fpRed   = np.memmap( fpRed_filename, dtype='float16', mode='w+',shape=(nrows,ncols))
  fpGreen = np.memmap( fpGreen_filename, dtype='float16', mode='w+',shape=(nrows,ncols))
  fpBlue  = np.memmap( fpBlue_filename, dtype='float16', mode='w+',shape=(nrows,ncols))
  fpPan   = np.memmap( fpPan_filename, dtype='float16', mode='w+',shape=(nrows,ncols))

  fpRed[:]   = dsMulti.GetRasterBand(1).ReadAsArray().astype(float)
  fpGreen[:] = dsMulti.GetRasterBand(2).ReadAsArray().astype(float)
  fpBlue[:]  = dsMulti.GetRasterBand(3).ReadAsArray().astype(float)
  fpPan[:]   = dsPan.GetRasterBand(1).ReadAsArray().astype(float)

  if dsMulti.RasterCount>3:
    fpNIR_filename   = os.path.join( outputDir , 'NIR.dat')
    fpNIR      = np.memmap( fpNIR_filename, dtype='float16', mode='w+',shape=(nrows,ncols))
    fpNIR[:]   = dsMulti.GetRasterBand(4).ReadAsArray().astype(float)
    imgFusion  = pansharpen( outputDir, fpRed,fpGreen,fpBlue,fpPan,fpNIR)
  else: 
    imgFusion  = pansharpen( outputDir, fpRed,fpGreen,fpBlue,fpPan )
  ...
  os.remove( fpRed_filename )
  os.remove( fpGreen_filename )
  os.remove( fpBlue_filename )
  os.remove( fpPan_filename )
  if dsMulti.RasterCount>3: os.remove( fpNIR_filename )
